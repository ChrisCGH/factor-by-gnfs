...
getstime()
{
   ...
   struct tms tbuf;
   times(&tbuf);
   return(1.0*tbuf.tms_stime/CLK_TCK);
#endif
}
...
)
{
   ...
   res = (long double) (n[i--]);
   for (; i; i--)
      res = res * fradix + (long double) (n[i]);
   if (n[0] > 0)
      return (res);
   return (-res);
}
...
)
{
   ...
   zsetlength(&a, (long) (strlen(in) / log16rad));
   a[0] = 1;
   a[1] = 0;
   while (in[d] != '\0')
   {
...
)
{
   ...
   if (non_initialized)
   {
      memset(ecm_coef, 0, ECM_MAXR * sizeof(verylong));
      memset(ecm_tex, 0, (ECM_MAXE + 1) * sizeof(verylong));
      memset(ecm_tey, 0, (ECM_MAXE + 1) * sizeof(verylong));
      memset(ecm_power, 0, ECM_MAXT * sizeof(verylong));
      memset(ecm_eval, 0, ECM_MAXT * sizeof(verylong));
      /*
...
#define MSK667 64
   if (first)
   {
      ...
      for (i = 0; i < 334; i++)
      {
         ...
         squtab[isq%371] |= MSK371;
         squtab[isq%517] |= MSK517;
         squtab[isq%559] |= MSK559;
         squtab[isq%589] |= MSK589;
         squtab[isq%615] |= MSK615;
         squtab[isq%629] |= MSK629;
